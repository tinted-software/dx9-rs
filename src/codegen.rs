...
const D3DSPR_TEXTURE: u32 = 3; // t# / oT#
const D3DSPR_RASTOUT: u32 = 4; // oPos, oFog, oPts
const D3DSPR_ATTROUT: u32 = 5; // oD0, oD1
const D3DSPR_SAMPLER: u32 = 16; // s#

// D3D9 Semantic Usages
        ...
        match &main_func.return_type {
            DataType::UserType(struct_name) => {
                if let Some(s_def) = self.structs.get(struct_name).cloned() {
                    for field in &s_def.fields {
                        let semantic = field.semantic.as_deref().unwrap_or("POSITION");
                        let (reg_type, index) = match semantic {
                            "POSITION" | "VPOS" => (D3DSPR_RASTOUT, 0),
                            s if s.starts_with("TEXCOORD") => {
                                let idx = s
                            ...
                            s if s.starts_with("COLOR") => {
                                let idx =
                                    s.strip_prefix("COLOR").unwrap().parse::<u32>().unwrap_or(0);
                                (D3DSPR_ATTROUT, idx)
                            }
                            _ => (D3DSPR_TEXTURE, output_reg_idx),
                        };
                        output_mapping
                            .insert(field.name.clone(), ResolvedRegister { reg_type, index });
                        if reg_type == D3DSPR_TEXTURE {
                            output_reg_idx += 1;
                        }
                    }
                }
            ...
            _ => {
                let semantic = main_func.return_semantic.as_deref().unwrap_or("COLOR");
                let (reg_type, index) = match semantic {
                    "POSITION" | "VPOS" => (D3DSPR_RASTOUT, 0),
                    s if s.starts_with("TEXCOORD") => {
                        let idx = s
                    ...
                    s if s.starts_with("COLOR") => {
                        let idx = s.strip_prefix("COLOR").unwrap().parse::<u32>().unwrap_or(0);
                        (D3DSPR_ATTROUT, idx)
                    }
                    _ => (D3DSPR_ATTROUT, 0),
                };
                output_mapping.insert("return".to_string(), ResolvedRegister { reg_type, index });
            }
        }
            ...
            Stmt::Return(expr_opt) => {
                if let Some(expr) = expr_opt {
                    ...
                    if let Some(out_reg) = output_mapping.get("return") {
                        ...
                        // Struct output return assignment handled differently, usually
                        // by local variable assignments or mapping.
                    }
                }
            }
            Stmt::Block(stmts) => {
